...
discourseModule(
  "Data Explorer Plugin | Integration | Component | upload-files-button",
  function (hooks) {
    setupRenderingTest(hooks);

    componentTest("it shows alert if a file with an unsupported extension was chosen", {
      skip: true,
      template: hbs`
        {{upload-files-button 
    ...
    componentTest("it shows alert if a file with an unsupported MIME type was chosen", {
      skip: true,
      template: hbs`
        {{upload-files-button 
          acceptedFileTypes=this.acceptedFileTypes 
